...
public class WorldGraphics {
	...
	private void loadTextures(GL10 gl, Context context) {
		_SkyBoxTexture = new GLTexture(gl,context, R.drawable.skybox);
		_Wall = new GLTexture(gl,context,R.drawable.starwall);
		_Floor = new GLTexture(gl,context,R.drawable.floor);
	}
	
	private void initSkyBox() {
		float d = (float)_grid_size;
	...
	public void drawWalls(GL10 gl) {
		...
		for(int Walls=0; Walls<4; Walls++) {
			...
			gl.glBindTexture(GL10.GL_TEXTURE_2D, _Wall.getTextureID());
	...
	public void drawSkyBox(GL10 gl) {
		...
		for(int i=0; i<6; i++) {
			gl.glBindTexture(GL10.GL_TEXTURE_2D, _SkyBoxTexture.getTextureID());
