...
public class GameState implements State {
    ...
    private final Lighting lights = new Lighting();
    ...
    public void init(Context ctx, GL10 gl, Video video) {
        ...
        // Initialize sounds
        if (mPrefs.playMusic()) SoundManager.playMusic(true);
    ...
    public void addTouchEvent(float x, float y) {
        ...
        if (player.getSpeed() > 0.0f) {
            if (initialState) {
                ...
                if (mPrefs.playMusic() && !SoundManager.isPlaying()) SoundManager.playMusic(true);
    ...
    private final long dtHist[] = new long[MAX_SAMPLES];
    ...
    private void render() {
        ...
        if (player.isVisible(camera)) player.draw(gl, timeCurrent, timeDelta, lights);
    ...
    public static void playPickup() {
        if (mPrefs.playSFX()) SoundManager.playSound(PICKUP_SOUND, 1.0f);
    }

    public static void playBoost() {
        if (mPrefs.playSFX()) SoundManager.playSound(BOOST_SOUND, 1.0f);
    }
    ...
    public void newGame() {
        ...
        world = new WorldGraphics(gl, mContext, mCurrentGridSize, player);
