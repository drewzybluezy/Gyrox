...
public class WorldGraphics {
    ...
    public WorldGraphics(GL10 gl, Context context, float gridSize, Player player) {
        // Save Grid Size
    ...
    public void drawFloorTextured(GL10 gl) {
        ...
        for (i = 0; i < (int) gridSize; i += l) {
            for (j = 0; j < (int) gridSize; j += l) {
                ...
                // information
                gl.glDrawElements(GL10.GL_TRIANGLE_STRIP, nIndices, GL10.GL_UNSIGNED_BYTE, indexBuffer);
    ...
    public void drawSkyBox(GL10 gl) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            gl.glPushMatrix();
            gl.glTranslatef(player.getXCoord(), player.getYCoord(), 0);

            gl.glDisableClientState(GL10.GL_LIGHTING);
            gl.glDrawElements(GL10.GL_TRIANGLES, nIndices, GL10.GL_UNSIGNED_BYTE, indexBuffer);
