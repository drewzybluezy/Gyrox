...
public class Camera {
	...
	float movement[] = new float[4]; // indices CAM_R, CAM_CHI, CAM_PHI,
										// CAM_PHI_OFFSET
	...
	private static final float cam_defaults[][] = {
			...
			{ CAM_FOLLOW_FAR_DIST, (float) Math.PI / 4.0f,
					(float) Math.PI / 72.0f }, // follow far
			{ CAM_FOLLOW_CLOSE_DIST, (float) Math.PI / 4.0f,
					(float) Math.PI / 72.0f }, // follow close
			{ CAM_FOLLOW_BIRD_DIST, (float) Math.PI / 4.0f,
					(float) Math.PI / 72.0f }, // birds-eye view
			{ CAM_COCKPIT_Z, (float) Math.PI / 8.0f, 0.0f }, // cockpit
			{ CAM_CIRCLE_DIST, (float) Math.PI / 3.0f, 0.0f } // free
	};

	private static final float camAngles[] = { (float) Math.PI / 2.0f, 0.0f,
			3.0f * (float) Math.PI / 2.0f, (float) Math.PI,
			2.0f * (float) Math.PI };

	public enum CamType {
		E_CAM_TYPE_CIRCLING, E_CAM_TYPE_FOLLOW, E_CAM_TYPE_FOLLOW_FAR, E_CAM_TYPE_FOLLOW_CLOSE, E_CAM_TYPE_BIRD, E_CAM_TYPE_COCKPIT, E_CAM_TYPE_MOUSE
	}
	...
	private void playerCamera(Player PlayerData, long CurrentTime, long dt) {
		...
		if (coupled == 1) {
			...
			/*
			 * if (time < PlayerData.TURN_LENGTH) { dir =
			 * PlayerData.getDirection(); ldir = PlayerData.getLastDirection();
			 * if (dir == 1 && ldir == 2) { dir = 4; } if (dir == 2 && ldir ==
			 * 1) { ldir = 4; } phi += ((PlayerData.TURN_LENGTH - time) *
			 * camAngles[ldir] + time camAngles[dir]) / PlayerData.TURN_LENGTH;
			 * } else { phi += camAngles[PlayerData.getDirection()]; }
			 */
		}
	...
	public void doLookAt(GL10 gl) {
		...
		gl.glTranslatef(-getCam().point[0], -getCam().point[1],
				-getCam().point[2]);
	}
